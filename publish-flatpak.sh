#!/usr/bin/env bash
# publish-flatpak.sh — build and publish Rubric to the personal flatpak repo
#
# Usage:
#   ./publish-flatpak.sh 0.18.0
#
...
# ── argument check ────────────────────────────────────────────────────────────
if [[ $# -ne 1 ]]; then
  echo "Usage: $0 <version>   e.g.  $0 0.18.0"
  exit 1
fi
VERSION="$1"
...
git push origin "v$VERSION" 2>/dev/null || true   # tag may already be pushed
