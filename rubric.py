...
"""
    ...
    def _build_minister_typst(self) -> str:
        ...
        _ms_p = config.preamble.get("manuscript", {})
        _ms_sz = _ms_p.get("size", 10)
        _ms_font = _ms_p.get("font", "").strip()
        _text_args = [f'size: {_ms_sz}pt']
        if _ms_font:
            _text_args.append(f'font: "{_ms_font}"')

        parts = [
            "// Leader's Order — generated by Rubric",
            '#set page(paper: "us-letter",'
            ' margin: (top: 0.75in, bottom: 0.75in, left: 0.7in, right: 0.7in))',
            f'#set text({", ".join(_text_args)})',
            '#set par(justify: false)',
            '',
            TYPST_SHARED,
            '',
