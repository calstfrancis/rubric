...
class MainWindow(Adw.ApplicationWindow):
    def __init__(self, **kw):
        ...
        self._setup_actions(); self._build_ui(); self._apply_density(); self._update_title(); self._exporter._update_tex_btn()
    ...
    def _build_ui(self):
        ...
        self.tex_btn.connect("clicked", lambda _: self._exporter.quick_export_typst())

        self.pdf_btn = Gtk.Button(icon_name="document-print-symbolic",
                                  tooltip_text="Compile to PDF via Typst (Ctrl+Shift+P)")
        self.pdf_btn.connect("clicked", lambda _: self._exporter.compile_typst_pdf())
        ...
        self._preview_update_mode = "on_save"  # "auto" | "on_save" | "manual"
    ...
    def _build_order_panel(self):
        ...
        self._order_hpaned.set_position(config.ui_panes.get("order_hpaned", 220))
        self._order_hpaned.set_vexpand(True)
    ...
    def _build_preview_panel(self) -> Gtk.Box:
        ...
        print_btn.connect("clicked", lambda _: self._exporter._print_bulletin_webkit())
        ...
        if visible:
            if not self._preview_paned_positioned:
                ...
                def _set_pos():
                    ...
                    if saved:
                        self._preview_paned.set_position(saved)
                    else:
    ...
    def _do_preview_update(self):
        ...
        try:
            if mode == "manuscript":
                html = self._exporter._build_manuscript_html()
            else:
                html = self._exporter._build_bulletin_html()
            ...
            if result.returncode == 0 and pdf_path.exists():
                ...
                if err:
                    ...
                    def _show_preview_err(msg=short, line=_line2):
                        _prefix2 = ""
                        if line:
                            _eidx2 = self._exporter._item_idx_from_error_line(line)
    ...
    def _popout_preview(self):
        ...
        if pdf_path and Path(pdf_path).exists():
            ...
            try:
                if mode == "manuscript":
                    wv.load_html(self._exporter._build_manuscript_html(), None)
                else:
                    wv.load_html(self._exporter._build_bulletin_html(), None)
            except Exception:
                pass
        tv.set_content(wv)
        win.set_content(tv)
